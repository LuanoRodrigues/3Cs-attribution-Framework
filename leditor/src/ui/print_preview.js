...
exports.createPrintPreviewModal = void 0;
const extension_page_break_js_1 = require("../extensions/extension_page_break.js");
const ensureStyles = () => {
    if (document.getElementById("leditor-print-preview-styles"))
        return;
...
const PAGE_BOUNDARY_KINDS = [
    "page",
    "section_next",
    "section_even",
    "section_odd"
];
const PAGE_BOUNDARY_SET = new Set(PAGE_BOUNDARY_KINDS);
const getBreakKindFromNode = (node) => {
    if (node.nodeType !== Node.ELEMENT_NODE)
        return null;
    const element = node;
    if (element.getAttribute("data-break") !== "true")
        return null;
...
const createBreakPlaceholder = (kind) => {
    const placeholder = document.createElement("div");
    placeholder.className = "leditor-break-placeholder";
    placeholder.dataset.breakKind = kind;
    placeholder.textContent = extension_page_break_js_1.BREAK_KIND_LABELS[kind] ?? "Break";
    return placeholder;
};
const buildPagesFromHtml = (html, host) => {
...
const createPrintPreviewModal = (editorHandle) => {
    ensureStyles();
...
exports.createPrintPreviewModal = createPrintPreviewModal;
