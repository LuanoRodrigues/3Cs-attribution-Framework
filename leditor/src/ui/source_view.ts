...
type SourceViewTab = "raw" | "html" | "markdown" | "json";
type SourceViewController = {
  open: (tab?: SourceViewTab) => void;
  close: () => void;
  toggle: () => void;
  isOpen: () => boolean;
};
...
const ensureSourceViewStyles = () => {
  if (document.getElementById("leditor-source-view-styles")) return;
...
export const createSourceViewModal = (editorHandle: EditorHandle): SourceViewController => {
  ...
  const textareaMap = new Map<TabKey, HTMLTextAreaElement>();
  const tabButtons = new Map<TabKey, HTMLButtonElement>();
  let activeTab: TabKey = "raw";
  ...
  const updateContent = () => {
    ...
    const rawHtmlValue = editorHandle.getEditor()?.view?.dom?.innerHTML ?? "";
    ...
    textareaMap.get("raw")!.value = rawHtmlValue;
...
export type { SourceViewController, SourceViewTab };
